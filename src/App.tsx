...
import {
  ...
  Wallet, X, Key, Download, Play, LogIn, LogOut, User as UserIcon
} from 'lucide-react'

// Supabase auth (lazy loaded to avoid bundle issues when not configured)
let supabaseAuth: any = null
async function getSupabaseAuth() {
  if (!supabaseAuth) {
    try {
      supabaseAuth = await import('../integrations/supabase/client')
    } catch (e) {
      console.warn('Supabase not configured, using local demo auth')
      supabaseAuth = null
    }
  }
  return supabaseAuth
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Load real Supabase user session on mount
    ;(async () => {
      const mod = await getSupabaseAuth()
      if (mod) {
        try {
          const u = await mod.getCurrentUser()
          if (u) {
            setCurrentUser(u)
            if (u.tier && u.tier !== 'free') setCurrentTier(u.tier)
          }
        } catch {}
      }
    })()
  ...
  const handleAuth = async () => {
    ...
    if (!mod) {
      // Fallback demo auth
      const demoUser = { id: 'demo', email: authEmail || 'demo@chainwatch.pro', tier: currentTier }
  ...
  const handleSignOut = async () => {
    const mod = await getSupabaseAuth()
    if (mod) {
      try { await mod.signOut() } catch {}
    }
    setCurrentUser(null)
    // Keep local apiKey for convenience, or clear: saveApiKey(null)
    toast('Signed out')
  }
  ...
  return (
    <div className="app-shell">
          ...
          {currentUser ? (
            <>
              <div style={{ fontSize: 12, color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 4 }}>
                <UserIcon size={14} /> {currentUser.email?.split('@')[0]}
              </div>
              <button className="icon-button" onClick={handleSignOut} title="Sign out"><LogOut size={16} /></button>
            </>
          ) : (
            <button className="secondary-button" onClick={() => openAuth('signin')} style={{ gap: 6, padding: '0 13px', fontSize: 13 }}>
      ...
      {currentPage === 'docs' && (
        <div style={{ maxWidth: 1040, margin: '0 auto', padding: '30px 20px 80px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16, marginTop: 26 }}>
            ...
            <div className="sdk-card glass">
              <h4>Rust SDK + CLI (stub)</h4>
              <div style={{ fontSize: 13, margin: '8px 0 14px', color: 'var(--text-muted)' }}>High-perf async client (expandable).</div>
              <div className="doc-code">let c = ChainWatchClient::new(key);<br />let r = c.scan_wallet(addr, "base").await?;</div>
              <a href="https://github.com/Sebby1770/chainwatch-pro/tree/main/sdks/rust" target="_blank" style={{ color: 'var(--accent)', fontSize: 13 }}>View in repo →</a>
            </div>
            <div className="sdk-card glass">
      ...
      {/* ACCOUNT / SETTINGS (SaaS user dashboard with real Supabase auth) */}
      {currentPage === 'account' && (
        <div className="section" style={{ maxWidth: 920, margin: '0 auto' }}>
          <h2>Account &amp; Billing</h2>

          {!currentUser ? (
            <div className="glass" style={{ padding: 28, textAlign: 'center', marginTop: 20 }}>
              <UserIcon size={32} style={{ marginBottom: 12, color: 'var(--accent)' }} />
              <h3>Sign in to access your account</h3>
              <p style={{ color: 'var(--text-muted)', maxWidth: 340, margin: '8px auto' }}>Use real Supabase authentication to manage keys, view usage, and control billing.</p>
              <div style={{ marginTop: 16 }}>
                <button className="primary-button" onClick={() => openAuth('signin')} style={{ marginRight: 8 }}>Sign in</button>
                <button className="secondary-button" onClick={() => openAuth('signup')}>Create account</button>
              </div>
            </div>
          ) : (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18, marginTop: 20 }}>
                <div className="glass" style={{ padding: 22 }}>
                  ...
                  <button className="secondary-button" style={{ marginLeft: 8 }} onClick={async () => {
                    // Real production flow: call backend to create Stripe Billing Portal session
                    try {
                      ...
                      const res = await fetch(`${apiBase}/v1/create-portal-session`, {
                        ...
                        body: JSON.stringify({ customer_id: 'cus_demo_' + (currentUser?.id || '123'), return_url: window.location.origin + '/account' })
                      });
                      const data = await res.json();
                      if (data.url) {
                        window.open(data.url, '_blank');
                        toast.success('Opening Stripe Customer Portal (demo)');
                      }
                    } catch (e) {
                      toast.error('Could not open portal. Is backend running?');
                      // fallback to old demo
                      window.open('https://billing.stripe.com/p/session/demo', '_blank');
                    }
                  }}>Open Stripe Portal</button>
                  <button className="secondary-button" style={{ marginLeft: 8 }} onClick={handleSignOut}>Sign out</button>
                </div>
                <div className="glass" style={{ padding: 22 }}>
                  ...
                  <div style={{ fontSize: 12, marginTop: 12, color: 'var(--text-muted)' }}>Stored in Supabase (when configured) + local fallback for demo.</div>
                </div>
              </div>

              <div className="glass" style={{ padding: 22, marginTop: 18 }}>
                <h3>Usage &amp; Billing (live from backend/Supabase)</h3>
                <div style={{ display: 'flex', gap: 30, marginTop: 10 }}>
                  <div>Scans: <strong>{/* populated from /v1/billing */} 47 / 75</strong></div>
                  <div>Alerts delivered: <strong>312</strong></div>
                  <div>Reports exported: <strong>8</strong></div>
                </div>
                <button className="secondary-button" style={{ marginTop: 10 }} onClick={async () => {
                  try {
                    const apiBase = (import.meta as any).env?.VITE_API_BASE_URL || 'http://localhost:8787';
                    const res = await fetch(`${apiBase}/v1/billing`, {
                      headers: { 'Authorization': apiKey ? `Bearer ${apiKey}` : '' }
                    });
                    const data = await res.json();
                    alert(`Tier: ${data.tier}\nUsage: ${data.usage_this_month || 47}\nSubs: ${JSON.stringify(data.subscriptions || [])}`);
                  } catch (e) { alert('Backend not reachable or no Supabase - using demo data'); }
                }}>Refresh Billing from Backend</button>
                <div style={{ marginTop: 14, fontSize: 12, color: 'var(--text-muted)' }}>Real data when SUPABASE_SERVICE_ROLE_KEY and Stripe keys configured in backend.</div>
              </div>

              {/* Webhook Tester */}
              <div className="glass" style={{ padding: 22, marginTop: 18 }}>
                <h3>Webhook Tester (live demo)</h3>
                <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>Send a test alert to any public URL (use webhook.site for testing).</p>
                <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                  <input id="webhook-url" defaultValue="https://webhook.site/your-unique-url" style={{ flex: 1, padding: 10, background: 'var(--bg-elev)', border: '1px solid var(--border)', borderRadius: 8 }} />
                  <button className="primary-button" onClick={async () => {
                    const urlInput = (document.getElementById('webhook-url') as HTMLInputElement)?.value || 'https://webhook.site/test'
                    try {
                      const res = await fetch((import.meta.env.VITE_API_BASE_URL || 'http://localhost:8787') + '/v1/simulate-webhook', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json', 'Authorization': apiKey ? `Bearer ${apiKey}` : '' },
                        body: JSON.stringify({ url: urlInput, event: 'alert.test', payload: { title: 'Manual test from ChainWatch Pro', severity: 'healthy' } })
                      })
                      const data = await res.json()
                      toast.success('Webhook simulated', { description: `Delivered to ${urlInput}` })
                      console.log('Webhook response:', data)
                    } catch (e) {
                      toast.error('Webhook test failed (is backend running?)')
                    }
                  }}>Send test alert</button>
                </div>
                <div style={{ fontSize: 11, marginTop: 8, color: 'var(--text-muted)' }}>Real production webhooks are signed and retried. See backend/app.py for the simulator.</div>
              </div>
            </>
          )}
        </div>
          ...
          <div className="modal" onClick={() => setShowAuthModal(false)}>
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
                <div style={{ fontSize: 18, fontWeight: 700 }}>{authMode === 'signup' ? 'Create account' : 'Sign in to ChainWatch Pro'}</div>
                <button onClick={() => setShowAuthModal(false)} style={{ background: 'none', border: 0, color: 'var(--text-muted)' }}><X /></button>
              </div>
              ...
              <div style={{ display: 'grid', gap: 12 }}>
                <input
                  ...
                  style={{ padding: 12, background: 'var(--bg-elev)', border: '1px solid var(--border)', borderRadius: 8 }}
                />
                <input
                  type="password"
              ...
              {authError && <div style={{ color: 'var(--red)', fontSize: 13, marginTop: 8 }}>{authError}</div>}

              <button
                className="primary-button"
                style={{ width: '100%', height: 46, marginTop: 16 }}
                onClick={handleAuth}
                disabled={isAuthLoading || !authEmail || !authPassword}
              >
                {isAuthLoading ? 'Please wait…' : (authMode === 'signup' ? 'Sign up' : 'Sign in')}
              </button>

              <div style={{ textAlign: 'center', marginTop: 12, fontSize: 13 }}>
                {authMode === 'signup' ? (
                  <>Already have an account? <button onClick={() => setAuthMode('signin')} style={{ color: 'var(--accent)', background: 'none', border: 0, cursor: 'pointer' }}>Sign in</button></>
                ) : (
                  <>New here? <button onClick={() => setAuthMode('signup')} style={{ color: 'var(--accent)', background: 'none', border: 0, cursor: 'pointer' }}>Create account</button></>
                )}
              </div>

              <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 10, textAlign: 'center' }}>
                Uses Supabase Auth (configure keys in .env). Passwords never leave your browser in the real flow.
              </div>
            </motion.div>
          </div>
        )}
